...
class SimulationService():
    ...
    def get_latest_simulation(self, uid):
        try:
            print(get_db().simulation_meta,  uid)
            
            result = get_db().simulation_meta.find_one({"object_uid": uid}, sort=[("version", -1)])
            return result
        except Exception as e:
            # Log the error
            print(f"An error occurred while getting the latest simulation: {e}")
            return None
