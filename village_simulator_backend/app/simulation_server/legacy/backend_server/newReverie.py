...
class NewReverieServer: 
  def __init__(self, 
    ...
    self.enviroment = SimulationService()
    self.timeline = self.enviroment.get_or_start_simulation_timeline(self.fork_sim_code, self.timeline_id)
    ...
    self.step = self.timeline['current_step']
    ...
    for persona_name in reverie_meta['persona_names']: 
      ...
      curr_persona = Persona(sim_code, timeline_id, persona_name)
  ...
  def save(self): 
    ...
    # service.update_simulation_timeline(self.sim_code, timeline_id, reverie_meta)
    # Save the personas.
    for persona_name, persona in self.personas.items(): 
      persona.save(self.step)
  ...
  def start_server(self, int_counter, timeline_id="original"): 
    """
    ...
    env = self.enviroment.get_or_start_simulation_timeline(self.sim_code, timeline_id)    
    if str(self.step) in env["steps"]:
      new_env = env["steps"][str(self.step)]
    else:
    ...
    for i in range(int_counter):
      ...
      for persona_name, persona in self.personas.items(): 
        ...
        # during this cycle. 
        new_tile = (new_env["persona"][persona_name]["x"], 
                    new_env["persona"][persona_name]["y"])
      ...
      for persona_name, persona in self.personas.items(): 
        ...
        movements["persona"][persona_name]["x"] = next_tile[0]
        movements["persona"][persona_name]["y"] = next_tile[1]
        movements["persona"][persona_name]["maze"] = self.maze.maze_name
      ...
      self.enviroment.add_new_simulation_step(self.sim_code, timeline_id, self.step, movements)
      self.save()
