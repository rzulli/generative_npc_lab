...
from ....database.persona import PersonaService

class Persona: 
  def __init__(self, sim_code, timeline_id, name):
    # PERSONA BASE STATE 
    ...
    latest_persona_state = self.service.get_latest_state(sim_code, timeline_id, self.name)
    if not latest_persona_state:
      print("PERSONA NÃO POSSUI STATE COM ESSES PARAMETROS ", sim_code, timeline_id, name)
      print("PROCURANDO TEMPLATE PARA", name)
      template = self.service.get_persona_template(name)
      print(template)
      self.persona_state = template
    else:
      print("LOADING STATE ", sim_code, timeline_id, name)
      self.persona_state = latest_persona_state["state"]

    print(self.persona_state["scratch"])
    # PERSONA MEMORY 
    # If there is already memory in folder_mem_saved, we load that. Otherwise,
    # we create new memory instances. 
    # <s_mem> is the persona's spatial memory. 
    
    self.s_mem = MemoryTree(self.persona_state["spatial_memory"])
    # <s_mem> is the persona's associative memory. 
    
    self.a_mem = AssociativeMemory(self.persona_state["embeddings"], self.persona_state["nodes"], self.persona_state["kw_strength"])
    # <scratch> is the persona's scratch (short term memory) space. 
    
    self.scratch = Scratch(self.persona_state["scratch"])
  ...
  def save(self, step): 
    ...
    associative = self.a_mem.save_to_db()
    ...
    scratch = self.scratch.save_to_db()
    data = {"scratch": scratch, "spatial_memory":spatial, "nodes": associative["nodes"],"kw":associative["kw"],"embeddings":associative["embeddings"],}
    self.service.add_persona_state(self.sim_code,self.timeline_id, self.name, step, data)
